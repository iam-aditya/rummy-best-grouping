...
type suit = 'c' | 'd' | 'h' | 's' | 'j' | string;
type groupId = number;
type uId = number;
type score = number;
type pos = number;
type isValid = boolean;
export type groupType = 'seq' | 'impSeq' | 'set' | 'inv';
type name = string;

type FormationResult ={
    isValid: isValid,
    score : score,
    groupIdToTypeMap : Map< groupId, groupType>,
    names : name[];
}
class Card{
    name : string
    val : number; // 2,3,...10,10,10,10,10
    rank : string; // 2,3,..,10,J,Q,K,A
    suit: suit; 
    pos: pos;  // 2,3,..10,11,12,13,14,
    joker : boolean = false;
    score : number;  // depends on the wild joker
    wildJoker : boolean = false;
    isUsed : boolean= false;
    groupId: groupId;
    uId : uId;

    usedAs: 'cd' | 'jk' | string = null;
    cardDeck : CardDeck
    constructor(id : name){
        ...
        if( this.rank == '' ) {
            ...
            if( this.rank == 'J') this.pos = 11;
            if( this.rank == 'Q') this.pos = 12;
            if( this.rank == 'K') this.pos = 13;
            if( this.rank == 'A') this.pos = 14;
        }
        else{
            this.pos =  this.val = parseInt( this.rank);
        } 
    }
    setCardInGroup(groupId : groupId , usedAsjoker : boolean){
        if( this.cardDeck?.cardManager?.foundValidDeclaration)return;
...
class CardDeck{
    cards : Array<Card>;
    pureSeqId : groupId =0;
    impureSeqId : groupId =10; // start from 10 for no overlapp
    setId : groupId =20;

    noOfCards =0;
    joker: Card;
    pureSeqs : Map< number , Card[] >
    impureSeqs : Map< number, Card[]> 
    sets : Map< number, Card[]> 
    cardManager: CardManager

    constructor(cards : Array< any > , joker: Card | name ){
    ...
    public setCardManager =( cardMan : CardManager) =>{
        this.cardManager = cardMan;
        return
    }
    public getCardByUid = ( uid : number) : Card| null => {
        /*
    ...
    // not used
    getPureSeq = ( ) : Map< number , Card[] > =>{
        console.log( 'pss',this.pureSeqs.size, this.pureSeqId );
        if( this.pureSeqs.size == this.pureSeqId) return this.pureSeqs;
    ...
    // not used
    getImPureSeq = ( ) : Map< number , Card[] > =>{
        if( this.impureSeqs.size == this.impureSeqId-10) return this.impureSeqs;
    ...
    // not used
    getSet = () : Map< number , Card[] > =>{
        if( this.sets.size == this.setId-20) return this.sets;
...
class CardManager {
    ...
    maxSizeOfSet = 4; // could be changed to 4 depending upon the rules
    sizeAndMaxElementToCombMap :  Map<number , number[][] > = new Map();
    minScore = 80;
    bestFormation : Map<uId, groupId>;
    foundValidDeclaration = false;
    bestFormationNames : Array<name> 
    ...
    private checkSet = ( cards : Array<Card>) : boolean =>{
        //console.log("check for set", cards.map(c =>c.name));
        if( cards.length < 3 || cards.length > this.maxSizeOfSet) return false;
        ...
        if( non_joker_ind==-1)return true; // found no non joker, so all jokers so set
        ...
        let ok = true;
        for(let i=0;i<cards.length; i++){
            if( !(cards[i].pos == comm_pos || cards[i].joker || cards[i].wildJoker)) ok = false;
        }
        //console.log(ok, "ok")
        if( ok) return true
    ...
    private checkImpureSeq = ( cards : Array<Card>) : boolean =>{
        ...
        if( jokersNeeded <= noOfJokers) return true;
        
        return false;
    }
    private checkPureSeq = ( cards : Array<Card>) : boolean =>{
        if( cards.length< 3)return false;
        const currentSuit  = cards[0].suit;
        //console.log("suit", currentSuit);
        if( !(currentSuit == 'c' || currentSuit == 'd' || currentSuit == 'h' || currentSuit == 's') )return false; // to not allow 3 or more jokers
        ...
        let pos_arr = cards.map( card=>{
            if(card.pos ==14 )return 1;
            else return card.pos;
        }).sort( (i,j) => i-j );
    ...
    private splitBySuit = ( cards : Array<Card> ) : Map<string, Array<Card> > =>{
        const groupedDeck  = new Map< string, Array<Card> >([
    ...
    private splitAndSortByPos = ( cards: Array<Card> ): Map<number, Array<Card> > =>{
        let groupedDeck = new Map< number, Array<Card> >();
        for( let p = 2; p<=14; p++){
    ...
    private splitAndSort = ( cards : Array<Card> ) : Map<string, Array<Card> > =>{
    ...
    private getSizeAndMaxElementToCombMap = ( s : number ) : Map< number, number[][] > =>{
    ...
    private getCombinationsMap = ( mxComb : number ) : Map< number, number[][] > =>{
        let arr = [];
        let sizeToCombMap : Map<number , number[][] > = new Map();
    ...
    private *generateSets( cardDeck : CardDeck ) : IterableIterator<Array<Card>> {
        ...
        allCards.forEach( ( card) =>{
            if( !card.isUsed && (card.joker || card.wildJoker) ) jokers.push(card);
        });
        ...
        let sizeToCombMap : Map<number , number[][] > = this.getCombinationsMap(jokers.length);
        ...
        for( let nj =0; nj<= mx_nj ; nj++){
            ...
            for( let v = 0; v<=14; v++ ){
                ...
                let uniqCards : Card[] = [];  // unique suits
                cards.forEach( card =>{
    ...
    private *generatePureSeq( cardDeck : CardDeck ) : IterableIterator<Array<Card>> {
        let allCards = cardDeck.cards;
        allCards = allCards.filter( card => !card.isUsed);
        const suitGroups =  this.splitAndSort(allCards);

        for(let i=0;i<4;i++){
            const cards : Card[]= suitGroups.get( this.suits[i] );
            let l =0, prevCard : Card= null;
            while( l < cards.length ){
                ...
                while( r< cards.length){
                    ...
                    else if( cards[r].pos >  prevVal + 1 )break;
    ...
    private combinations <T> (arr : T[], m : number) {
        const result : T[][]= [];
    ...
    private checkDisjoint( group1 :number[] ,  group2 :number[]) : boolean {
        //group1.
    ...
    private *generateAnySeq( cardDeck : CardDeck ) : IterableIterator<Array<Card>> { 
        let allCards = cardDeck.cards;
        ...
        allCards.forEach( ( card, id) =>{
            if( !card.isUsed && (card.joker || card.wildJoker) ) jokersIndex.push(card.uId);
        });
        ...
        for(let i=0;i<4;i++){
            const cards : Card[]= suitGroups.get( this.suits[i] ); // group of cards of one suit
            let l =0, prevCard : Card= null;
    ...
    private setbestFormation = ( cardDeck : CardDeck) =>{
    ...
    private findPureImpureSeqOrSet = (cardDeck: CardDeck) =>{
        let res = 0;
        
        if( this.foundValidDeclaration )return;
    ...
    public checkDeclarationAndScore = ( cards : Array<Card|name>, joker : Card | name  ) : FormationResult =>{
        ...
        */
        ...
        const groupIdCardMap : Map< groupId, Card[]> = new Map( )
        ...
        let pureSeqGroupIds : groupId[] =[];
        let imPureSeqGroupIds : groupId[] =[];
        let setGroupIds : groupId[] = [];
        let noOfGroups = 0;
        ...
        let groupIdToTypeMap : Map< groupId, groupType> = new Map();
        ...
        if( count.pureSeq >= 1 && count.impureSeq + count.pureSeq >=2 && (count.impureSeq + count.pureSeq + count.sets == noOfGroups) ){
            ...
            isValid = false;
            if( directPenalty )score = 80;
            else score = Math.min( score, cardDeck.getScore() );
        }
        
        const bestFormation: FormationResult = {
            isValid,
            groupIdToTypeMap,
            score,
    ...
    public bestGrouping = ( cards : Array<Card | name> , joker : Card|name ) : FormationResult =>{
        ...
        for( const p1 of pureSeqIterator){
            ...
            if( this.foundValidDeclaration ) break;
        ...
        const groupIdToTypeMap : Map< groupId, groupType> = new Map();
        ...
        const formationResult : FormationResult= {
    ...
    // to be implemented
    public bestCardToDiscard = ( cards : Array<Card| name> , joker : Card|name ) : Card =>{
        // given a deck of 14 cards find the card to discard for best formation
        return;
    }

    // to be implemented
    public bestNextMove = ( cards : Array<Card | name> , joker : Card|name ) : Card =>{
...
const cards1 = new Array<Card>(
    new Card('8d'), new Card('9d'), new Card('Jd'), new Card('Qd'), new Card('Kd'), new Card('Ad'), new Card('2d'), new Card('3d'), 
    new Card('j'), new Card('5s'), new Card('3s'),
    new Card('7c'), new Card('8c'), new Card('9c'), new Card('10c'),
    new Card('As'), new Card('2s'), new Card('3s'),
)

const cards2 = new Array<Card>(
    new Card('3d'), new Card('9d'),  new Card('8c'), new Card('9c'),
    new Card('2h'), new Card('10d'), new Card('10c'), new Card('Jc'),
    new Card('5d'), new Card('j'), new Card('5s'), new Card('5h'), new Card('j')
)

const cards3 = new Array<Card>(
    new Card('2s'), new Card('4s'),  new Card('5s'), new Card('7d'),
    new Card('8d'), new Card('9d'), new Card('Ac'), new Card('Kc'),
    new Card('Qc'), new Card('6c'), new Card('6c'), new Card('Qd'), new Card('8c')
)

const cardTestCases = [
    new Array<Card>(
        new Card('3.d'), new Card('9.d'),  new Card('8.c'), new Card('9.c'),
        new Card('2.h'), new Card('10.d'), new Card('10.c'), new Card('J.c'),
        new Card('5.d'), new Card('.j'), new Card('5.s'), new Card('5.h'), new Card('.j')
    ),
    new Array<Card>(
        new Card('2.s'), new Card('4.s'),  new Card('5.s'), new Card('7.d'),
        new Card('8.d'), new Card('9.d'), new Card('A.c'), new Card('K.c'),
        new Card('Q.c'), new Card('6.c'), new Card('6.c'), new Card('Q.d'), new Card('8.c')
    ),
    new Array<Card>(
        new Card('7.h'), new Card('J.d'),  new Card('J.h'), new Card('Q.c'),
        new Card('9.d'), new Card('9.h'), new Card('3.c'), new Card('4.s'),
        new Card('10.d'), new Card('10.h'), new Card('7.c'), new Card('9.s'), new Card('9.s')
    ),
]
...
/*
console.log(cm.checkDeclarationAndScore(new Array(
...

{
  ...
  groupIdToTypeMap: Map(4) { 11 => 'impSeq', 12 => 'impSeq', 1 => 'seq', 21 => 'set' }
}
{
  isValid: false,
  score: 18,
  ...
  groupIdToTypeMap: Map(4) { 11 => 'impSeq', 2 => 'seq', 1 => 'seq', null => 'inv' }
}
{
  isValid: false,
  score: 49,
  ...
  groupIdToTypeMap: Map(3) { undefined => 'inv', 1 => 'seq', 2 => 'seq' }
}

 */

export {Card, CardDeck, CardManager}
